...
into AWX/AAP at the platform level, the components involved, and the data flow
of a single inventory sync.
...
| **AWX / AAP Controller** | Runs inventory syncs and job templates. Manages credentials, projects, inventories, EEs. |
| **Custom Credential Type** | Defined by `extensions/awx/credential_types/akeyless_cert_auth.yml`. Declares the cred fields (`akeyless_api_url`, `access_id`, `cert_data`, `key_data`) and the **injectors** that write the cert/key to tempfiles and set `AKEYLESS_*` env vars at job-run time. |
...
| **Inventory plugin** (`akeyless.awx_integration.akeyless`) | Reads its config from the YAML, reads auth from env vars set by the credential injectors, calls Akeyless, and attaches secret values as `host_vars` / `group_vars` on the inventory. |
| **`akeyless.secrets_management` collection** | Hard runtime dep. Provides `AkeylessAuthenticator` and `AkeylessHelper` in `module_utils`. The inventory plugin imports these rather than re-implementing the API client. |
...
3. The Custom Credential Type's **injectors** fire inside that container:
   - `cert_data` and `key_data` PEM payloads are written to tempfiles whose
     paths are `{{ tower.filename.cert }}` and `{{ tower.filename.key }}`.
...
5. The plugin's `verify_file()` checks the filename ends with `akeyless.yml`
   or `akeyless.yaml`. If not, the source is skipped silently. This is why all
   examples use that suffix.
6. The plugin's `parse()` runs:
   1. `_build_auth_options()` reads each option via `get_option()`. Options
      ...
      auto-resolve from the env vars the credential injected, so users do not
      put auth fields in YAML.
   2. `_resolve_cert_material()` base64-encodes the cert and key files into
      `cert_data` / `key_data` for the authenticator.
   ...
   4. If `secret_path_prefix` is set, `_discover_via_prefix()` calls
      ...
      `{name, var}` mappings.
   5. Any explicit `secrets:` entries in the YAML are appended to that list.
   ...
   7. `_populate_inventory()` attaches each value to the configured
      `hosts` / `groups` / `default_group` as a host or group variable.
...
8. Any job template that references this inventory now sees the secrets as
   ordinary `host_vars` / `group_vars`. Playbooks contain no Akeyless code.

## Why an inventory plugin (not a credential plugin)

AWX's first-class "Credential Plugin" / "External Secret Management Source"
mechanism — the one HashiCorp Vault, CyberArk, etc. use — requires
modifications inside AWX itself. AWX upstream is in a refactor and has not
been accepting new credential plugins (as of 2024-07).

An inventory plugin combined with a Custom Credential Type achieves the
same end-state without depending on the frozen upstream:
...
| Where the secret value materializes | A single `Credential` instance per secret | `host_vars` / `group_vars` on every host in the inventory |
...
install project-level `requirements.txt` for inventory updates. The SDK and
both collections must be baked into the EE up front.
...
- The EE container running the inventory sync must reach
  `https://api.akeyless.io` over HTTPS (port 443).
- That is the only outbound call the inventory plugin makes. There is no
  customer-side gateway in this flow.

```
AWX EE container --HTTPS:443--> api.akeyless.io
```

> **Why not a customer gateway?** Cert-auth handshakes must terminate at the
> Akeyless SaaS. Most customer gateway ingresses terminate TLS at the edge and
> do not forward TLS client certs through to the gateway pod, so cert-auth
> handshakes fail there. See [`04-akeyless-cert-auth.md`](04-akeyless-cert-auth.md)
> for the endpoint distinction in detail.
